...
static void fft_commit_params(fft_t* p_fft_inst)
{
    ...
    // Write to the FWD FFT:
	reg  = (p_fft_inst->scale_sch_FWD     << FFT_SCALE_SCH_SHIFT) & FFT_SCALE_SCH_MASK;
	reg |= (FFT_FORWARD                   << FFT_FWD_INV_SHIFT)   & FFT_FWD_INV_MASK;
	reg |= (int_log2(p_fft_inst->num_pts) << FFT_NUM_PTS_SHIFT)   & FFT_NUM_PTS_MASK;
	XGpio_DiscreteWrite(&p_fft_inst->periphs.gpio_inst_FWD, 1, reg);


    // Write to the INV FFT:
	reg = 0;
	reg  = (p_fft_inst->scale_sch_INV     << FFT_SCALE_SCH_SHIFT) & FFT_SCALE_SCH_MASK;
	reg |= (FFT_INVERSE                   << FFT_FWD_INV_SHIFT)   & FFT_FWD_INV_MASK;
	reg |= (int_log2(p_fft_inst->num_pts) << FFT_NUM_PTS_SHIFT)   & FFT_NUM_PTS_MASK;
	XGpio_DiscreteWrite(&p_fft_inst->periphs.gpio_inst_INV, 1, reg);
}

// Public functions
fft_t* fft_create(  int gpio_device_id_FWD, 
                    int gpio_device_id_INV, 
                    int dma_device_id, 
                    XScuGic* p_intc_inst, 
                    int s2mm_intr_id, 
                    int mm2s_intr_id)
{
	...
	status = init_gpio(&p_obj->periphs.gpio_inst_FWD, gpio_device_id_FWD);
	if (status != FFT_SUCCESS)
	{
		xil_printf("ERROR! Failed to initialize GPIO FWD.\n\r");
		fft_destroy(p_obj);
		return NULL;
	}
	status = init_gpio(&p_obj->periphs.gpio_inst_INV, gpio_device_id_INV);
	if (status != FFT_SUCCESS)
	{
		xil_printf("ERROR! Failed to initialize GPIO INV.\n\r");
		fft_destroy(p_obj);
		return NULL;
	}
	...
	status = fft_set_num_pts(p_obj, INIT_NUM_FFT_PTS);
	if (status != FFT_SUCCESS)
	{
...
void fft_set_scale_sch_FWD(fft_t* p_fft_inst, int scale_sch)
{
	p_fft_inst->scale_sch_FWD = scale_sch;
}
void fft_set_scale_sch_INV(fft_t* p_fft_inst, int scale_sch)
{
	p_fft_inst->scale_sch_INV = scale_sch;
}

int fft_get_scale_sch_FWD(fft_t* p_fft_inst)
{
	return (p_fft_inst->scale_sch_FWD);
}
int fft_get_scale_sch_INV(fft_t* p_fft_inst)
{
	return (p_fft_inst->scale_sch_INV);
}
...
void fft_print_params(fft_t* p_fft_inst)
{
	...
	xil_printf("scale_sch_FWD = 0x%X\n\r", p_fft_inst->scale_sch_FWD);
	xil_printf("scale_sch_INV = 0x%X\n\r", p_fft_inst->scale_sch_INV);
}
...
void fft_print_result_buf(fft_t* p_fft_inst, int num_pts_to_print)
{
	...
	{
		num_pts_to_print = p_fft_inst->num_pts;
	}
	// Local variables
	int          ii = 0;
	char         str[25]; // Large enough to hold 2 ints plus extra characters
